...
Tests for phone cleaning, demo mode dispatch, and mock sandbox flows.
...
class TestWhatsAppDelivery:
    ...
    @patch("database.AsyncSessionLocal")
    @patch("config.settings")
    def test_whatsapp_delivery_demo_mode(self, mock_settings, mock_session):
        """Should run in demo mode and update DB status without external API calls."""
        mock_settings.R2_PUBLIC_URL = "https://r2.vaanibank.in"
        ...
        # Test: 10-digit number should be cleaned and processed in demo mode
        import asyncio
        asyncio.run(_send_whatsapp_background(
            summary_id=1,
        ...
        # Verify DB commit was called (status update happens in demo mode too)
